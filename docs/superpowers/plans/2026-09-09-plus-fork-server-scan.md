...
- [ ] Live local reverse proof (spec §5, VLESS reverse proxy — S1 found the legacy block is being removed from the cores): two `ServerCore` instances in temp data dirs — portal with `interconn` (vless+tcp+none on 127.0.0.1:P1, one client carrying `reverse.tag`) and `users` (vless+ws+none on 127.0.0.1:P2, one client); bridge with `via:'link'` = the interconn client's link and `exit: direct`; a throwaway client core dialling P2's link; `httpThroughProxy` → `ok`, and the bridge's `exit` outbound counters moved while the portal's `exit` did not.
